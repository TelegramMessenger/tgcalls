...
void GetLowestAndHighestQualityStreamIndixes(
  ...
  *lowest_quality_stream_idx =
      (int)std::distance(streams.begin(), lowest_highest_quality_streams.first);
  *highest_quality_stream_idx =
      (int)std::distance(streams.begin(), lowest_highest_quality_streams.second);
}
...
CustomSimulcastEncoderAdapter::EncoderContext::EncoderContext(
...
void CustomSimulcastEncoderAdapter::EncoderContext::Release() {
  if (encoder_) {
    encoder_->Release();
...
CustomSimulcastEncoderAdapter::StreamContext::StreamContext(
    CustomSimulcastEncoderAdapter* parent,
...
CustomSimulcastEncoderAdapter::StreamContext::StreamContext(StreamContext&& rhs)
    : parent_(rhs.parent_),
...
std::unique_ptr<CustomSimulcastEncoderAdapter::EncoderContext>
CustomSimulcastEncoderAdapter::StreamContext::ReleaseEncoderContext() && {
  encoder_context_->Release();
  return std::move(encoder_context_);
}

void CustomSimulcastEncoderAdapter::StreamContext::OnKeyframe(Timestamp timestamp) {
  is_keyframe_needed_ = false;
...
bool CustomSimulcastEncoderAdapter::StreamContext::ShouldDropFrame(
    Timestamp timestamp) {
  if (!framerate_controller_) {
    return false;
  }
...
CustomSimulcastEncoderAdapter::StreamContext::OnEncodedImage(
...
void CustomSimulcastEncoderAdapter::StreamContext::OnDroppedFrame(
    DropReason /*reason*/) {
  RTC_CHECK(parent_);  // If null, this method should never be called.
...
CustomSimulcastEncoderAdapter::CustomSimulcastEncoderAdapter(VideoEncoderFactory* factory,
    ...
    : CustomSimulcastEncoderAdapter(factory,
                              nullptr,
                              format,
...
CustomSimulcastEncoderAdapter::CustomSimulcastEncoderAdapter(
    ...
    : inited_(0),
      ...
      hardware_encoder_factory_(hardware_factory),
...
void CustomSimulcastEncoderAdapter::SetFecControllerOverride(
    FecControllerOverride* /*fec_controller_override*/) {
  // Ignored.
}

int CustomSimulcastEncoderAdapter::Release() {
...
int CustomSimulcastEncoderAdapter::InitEncode(
  ...
  std::unique_ptr<EncoderContext> encoder_context = FetchOrCreateEncoderContext(
      /*is_lowest_quality_stream=*/(
          ...
          codec_.simulcastStream[lowest_quality_stream_idx].active), false);
  ...
  bool separate_encoders_needed = true;
  ...
  for (int stream_idx = 0; stream_idx < total_streams_count_; ++stream_idx) {
    ...
    if (encoder_context == nullptr) {
      encoder_context = FetchOrCreateEncoderContext(
        /*is_lowest_quality_stream=*/stream_idx == lowest_quality_stream_idx, /*is_highest_quality_stream=*/stream_idx == highest_quality_stream_idx);
    ...
    CustomSimulcastEncoderAdapter* parent = stream_idx > 0 ? this : nullptr;
...
int CustomSimulcastEncoderAdapter::Encode(
...
int CustomSimulcastEncoderAdapter::RegisterEncodeCompleteCallback(
...
void CustomSimulcastEncoderAdapter::SetRates(
...
void CustomSimulcastEncoderAdapter::OnPacketLossRateUpdate(float packet_loss_rate) {
  for (auto& c : stream_contexts_) {
    c.encoder().OnPacketLossRateUpdate(packet_loss_rate);
  }
}

void CustomSimulcastEncoderAdapter::OnRttUpdate(int64_t rtt_ms) {
  for (auto& c : stream_contexts_) {
    c.encoder().OnRttUpdate(rtt_ms);
  }
}

void CustomSimulcastEncoderAdapter::OnLossNotification(
...
EncodedImageCallback::Result CustomSimulcastEncoderAdapter::OnEncodedImage(
    size_t stream_idx,
...
void CustomSimulcastEncoderAdapter::OnDroppedFrame(size_t stream_idx) {
  // Not yet implemented.
}
...
std::unique_ptr<CustomSimulcastEncoderAdapter::EncoderContext>
CustomSimulcastEncoderAdapter::FetchOrCreateEncoderContext(
    bool is_lowest_quality_stream, bool is_highest_quality_stream) const {
    bool prefer_temporal_support = false;/*fallback_encoder_factory_ != nullptr &&
                                    is_lowest_quality_stream &&
  ...
  std::unique_ptr<CustomSimulcastEncoderAdapter::EncoderContext> encoder_context;
  if (encoder_context_iter != cached_encoder_contexts_.end()) {
    encoder_context = std::move(*encoder_context_iter);
    cached_encoder_contexts_.erase(encoder_context_iter);
  } else {
    std::unique_ptr<VideoEncoder> primary_encoder;
    if (!primary_encoder && is_highest_quality_stream && hardware_encoder_factory_) {
      primary_encoder = hardware_encoder_factory_->CreateVideoEncoder(video_format_);
    }
    if (!primary_encoder) {
      primary_encoder = primary_encoder_factory_->CreateVideoEncoder(video_format_);
    }
    ...
    encoder_context = std::make_unique<CustomSimulcastEncoderAdapter::EncoderContext>(
...
webrtc::VideoCodec CustomSimulcastEncoderAdapter::MakeStreamCodec(
    const webrtc::VideoCodec& codec,
    int stream_idx,
  ...
  // CustomSimulcastEncoderAdapter is used. This allows multiple encodings of L1Tx,
...
VideoEncoder::EncoderInfo CustomSimulcastEncoderAdapter::GetEncoderInfo() const {
  ...
  encoder_info.implementation_name = "CustomSimulcastEncoderAdapter";
  ...
  if (stream_contexts_.empty()) {
    ...
    std::unique_ptr<CustomSimulcastEncoderAdapter::EncoderContext> encoder_context =
        FetchOrCreateEncoderContext(/*is_lowest_quality_stream=*/true, false);
    if (encoder_context == nullptr) {
      return encoder_info;
    }
