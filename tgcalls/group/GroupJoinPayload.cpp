...
absl::optional<GroupParticipantVideoInformation> GroupParticipantVideoInformation::parse(std::string const &data) {
    ...
    GroupParticipantVideoInformation result;

    if (const auto endpointId = parseString(json.object_items(), "endpoint")) {
        result.endpointId = endpointId.value();
    } else {
        return absl::nullopt;
    }

    const auto ssrcGroups = json.object_items().find("ssrc-groups");
    if (ssrcGroups == json.object_items().end() || !ssrcGroups->second.is_array()) {
        return absl::nullopt;
    }
    for (const auto &ssrcGroup : ssrcGroups->second.array_items()) {
        ...
        const auto sources = ssrcGroup.object_items().find("sources");
        if (sources == ssrcGroup.object_items().end() || !sources->second.is_array()) {
            return absl::nullopt;
        }
        for (const auto &source : sources->second.array_items()) {
            if (!source.is_number()) {
                return absl::nullopt;
            }
            int32_t signedValue = source.int_value();
            uint32_t unsignedValue = *(uint32_t *)&signedValue;
            parsedGroup.ssrcs.push_back(unsignedValue);
        }

        const auto semantics = ssrcGroup.object_items().find("semantics");
        if (semantics == ssrcGroup.object_items().end() || !semantics->second.is_string()) {
            return absl::nullopt;
        }
        parsedGroup.semantics = semantics->second.string_value();

        result.ssrcGroups.push_back(std::move(parsedGroup));
    }
